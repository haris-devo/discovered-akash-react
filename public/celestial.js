...
Celestial.display = function(config) {
  ...
  var ratio = projectionSetting.ratio,
      ...
      canvasheight = isNumber(cfg.background.width) ? width + cfg.background.width : width,
...
function form(cfg) {
  ...
  col.append("input").attr("type", "button").attr("id", "download-png").attr("value", "PNG Imagess").on("click", function() {
    var a = d3.select("body").append("a").node(), 
        canvas = document.querySelector(parentElement + ' canvas');
    a.download = getFilename(".png");
    a.rel = "noopener";
    a.href = canvas.toDataURL('image/png').replace('image/png', 'image/octet-stream');
    window.location.href=`http://localhost:3000/edit/${canvas.toDataURL('image/png').replace('image/png', 'image/octet-stream')}`
    // a.click();
    d3.select(a).remove();
  });
